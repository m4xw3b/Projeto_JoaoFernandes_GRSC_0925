...
dominio=""
host_server=""
ip_server="" # Variável que conterá APENAS o IP limpo (sem /24)
host_ftp=""
ip_ftp="" # Variável que conterá APENAS o IP do FTP (sem /24)
inter=""
ipad="" # Variável para o IP com CIDR (ex: 192.168.1.1/24)
gat=""
dns=""
opcao=0
...
# Função SIMPLES para remover o prefixo CIDR (/24, /16, etc.)
clean_ip() {
    # Usa 'awk -F/' para dividir pelo caractere '/' e retorna o primeiro campo (o IP)
...
read -r serv_dns
# *** Limpa o IP do servidor ***
ip_server=$(clean_ip "$serv_dns")
...
# Usando <<-EOL (com hífen) para permitir tabulação no conteúdo
...
@ IN SOA ns1.$dominio. admin.$dominio. (
	2025110705 ; Serial (Atualizado)
	3600 1800 1209600 86400 ) ; Refresh Retry Expire Minimum TTL
